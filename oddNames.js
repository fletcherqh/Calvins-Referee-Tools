...
/* global dice, Confluxor, feminineCensusNames, masculineCensusNames, epithets, fighterNames */
...
// === Thief names & epithets (ensured) ===
(function(scope){
  var oddNames = scope.oddNames = scope.oddNames || {};
  // Word lists (adjective, noun)
  var THIEF_ADJECTIVES = [
    "Hood","Hooded","Cloak","Cloaked","Danger","Grey","Hidden","Silver",
    "Shadow","Shade","Dash","Slim","Swift","Secret","Silent","Spy",
    "Crouching","Whisper","Sharp","Split","Jack","Cold"
  ...
  // Epithet: 50% MU; else pick neutral/chaotic fighter epithets. Returns "the <Epithet>"
  oddNames.thiefEpithet = function(){
    try {
      var mu = (typeof magicUserEpithets !== "undefined" && magicUserEpithets.length) ? magicUserEpithets : null;
      var nf = (typeof neutralFightingManAmazonEpithets !== "undefined" && neutralFightingManAmazonEpithets.length) ? neutralFightingManAmazonEpithets : null;
      var cf = (typeof chaoticFightingManAmazonEpithets !== "undefined" && chaoticFightingManAmazonEpithets.length) ? chaoticFightingManAmazonEpithets : null;
      var epi = null;
      if (Math.random() < 0.5 && mu) {
        epi = pick(mu);
      } else {
        var pools = [];
        if (nf) pools.push(nf);
        if (cf) pools.push(cf);
        if (pools.length) epi = pick(pools)[Math.floor(Math.random()*pick(pools).length)];
        // Above line picks pool twice; fix: choose pool once.
      }
    } catch(e){ epi = null; }
    if (!epi) {
      // choose pool properly if not yet selected
      var pools2 = [];
      if (typeof neutralFightingManAmazonEpithets !== "undefined" && neutralFightingManAmazonEpithets.length) pools2.push(neutralFightingManAmazonEpithets);
      if (typeof chaoticFightingManAmazonEpithets !== "undefined" && chaoticFightingManAmazonEpithets.length) pools2.push(chaoticFightingManAmazonEpithets);
      if (pools2.length) {
        var pool = pools2[Math.floor(Math.random()*pools2.length)];
        epi = pool[Math.floor(Math.random()*pool.length)];
      } else if (typeof magicUserEpithets !== "undefined" && magicUserEpithets.length) {
        epi = magicUserEpithets[Math.floor(Math.random()*magicUserEpithets.length)];
      }
    }
